...
calculateBudgetAnalysis(
  ...
  const safeSupplierCount = Math.max(1, Math.floor(supplierCount || defaults.defaultSupplierCount));
  const safeHourlyRate = Math.max(0, hourlyRate || defaults.defaultHourlyRate);
  ...
  const safeResponsivenessStrategy = Array.isArray(responsivenessStrategy) ? responsivenessStrategy : this.defaultResponsivenessStrategy;

  // Calculate Panel 3 tool costs using new three-component structure
  const toolDeployments = safeHrddStrategy.map((coverage, index) => {
    ...
    // Three cost components per tool
    const annualProgrammeCost = safeToolAnnualCosts[index] || 0;
    const perSupplierCost = safeToolPerSupplierCosts[index] || 0;
    const internalHoursPerSupplier = safeToolInternalHours[index] || 0;

    const totalAnnualProgrammeCost = annualProgrammeCost; // Fixed cost regardless of supplier count
    const totalPerSupplierCost = suppliersUsingTool * perSupplierCost;
    const totalInternalCost = suppliersUsingTool * internalHoursPerSupplier * safeHourlyRate;
    const totalToolCost = totalAnnualProgrammeCost + totalPerSupplierCost + totalInternalCost;
    ...
    return {
      ...
      annualProgrammeCost,
      perSupplierCost,
      internalHoursPerSupplier,
      totalAnnualProgrammeCost,
      totalPerSupplierCost,
      totalInternalCost,
      totalToolCost
    };
  });

  // Calculate Panel 4 response method costs
  const responseDeployments = safeResponsivenessStrategy.map((allocation, index) => {
    const allocationPercent = allocation / 100; // Convert percentage to decimal
    const effectiveSuppliersForMethod = Math.ceil(safeSupplierCount * allocationPercent);
    const internalHoursPerSupplier = safeResponseInternalHours[index] || 0;
    const totalInternalCost = effectiveSuppliersForMethod * internalHoursPerSupplier * safeHourlyRate;

    return {
      methodIndex: index,
      methodName: this.responsivenessLabels[index],
      allocation: allocation,
      allocationPercent,
      effectiveSuppliersForMethod,
      internalHoursPerSupplier,
      totalInternalCost
    };
  });

  // Calculate totals using new formula:
  // Total Cost = Σ(Annual Programme Costs) + Σ(Per Supplier Costs × Suppliers Using Tool) + 
  //              Σ(Tool Internal Hours × Suppliers Using Tool × Hourly Rate) +
  //              Σ(Response Internal Hours × Allocated Suppliers × Hourly Rate)
  
  const totalAnnualProgrammeCosts = toolDeployments.reduce((sum, tool) => sum + tool.totalAnnualProgrammeCost, 0);
  const totalPerSupplierCosts = toolDeployments.reduce((sum, tool) => sum + tool.totalPerSupplierCost, 0);
  const totalToolInternalCosts = toolDeployments.reduce((sum, tool) => sum + tool.totalInternalCost, 0);
  const totalResponseInternalCosts = responseDeployments.reduce((sum, response) => sum + response.totalInternalCost, 0);
  
  const totalExternalCost = totalAnnualProgrammeCosts + totalPerSupplierCosts; // External costs = programme + per-supplier
  const totalInternalCost = totalToolInternalCosts + totalResponseInternalCosts; // Internal costs = tool hours + response hours
  ...
  return {
    ...
    // Cost breakdowns
    totalAnnualProgrammeCosts,
    totalPerSupplierCosts, 
    totalToolInternalCosts,
    totalResponseInternalCosts,
    totalExternalCost,
    totalInternalCost,
    totalBudget,
    ...
    currentResponsivenessAllocation: safeResponsivenessStrategy
  };
}
