...
// CSV data as string (from the provided file)
...
async function importData() {
  try {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      const country = {
        name: parsedValues[0].replace(/"/g, ''),
        isoCode: parsedValues[1],
        itucRightsRating: parseFloat(parsedValues[2]),
        corruptionIndex: parseFloat(parsedValues[3]),
        migrantWorkerPrevalence: parseFloat(parsedValues[4]),
        wjpIndex: parseFloat(parsedValues[5]),
        walkfreeSlaveryIndex: parseFloat(parsedValues[6]),
        baseRiskScore: parseFloat(parsedValues[7])
      };
